...
function LoginForm() {
  ...
  const handleOAuthSignIn = async (provider: OAuthProvider) => {
    try {
      ...
      console.error(`[Login] ${provider.name} sign-in failed:`, err);
      ...
      <div className="w-full max-w-md space-y-8 rounded-xl bg-white p-8 shadow-lg">
        ...
        <div className="space-y-4">
          {OAUTH_PROVIDERS.map((provider) => {
            ...
            const baseClasses = `flex w-full items-center justify-center gap-3 rounded-lg px-4 py-3 transition ${
              isGoogle
                ? 'border border-gray-300 hover:bg-gray-50'
                : `${provider.bgColor} hover:opacity-90`
            } ${provider.textColor}`;
            ...
            return (
              ...
              >
                {provider.icon}
                {isLoading ? `Signing in with ${provider.name}...` : `Sign in with ${provider.name}`}
              </button>
            );
          })}
        </div>
